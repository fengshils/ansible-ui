...
use \app\model\Playbook;

class AnsiblePlaybook extends BaseController
{
    ...
    public  function playbook(){
        if(\request()->isPost()){
            ...
            if(input('post.')['group']){
                ...
                $playbook = input('post.')['playbook'];
                $playbook_path = Playbook::where('name', $playbook)->find()['path'];
                $data = 'ansible -i /tmp/hosts '. input('post.')['group'] . ' '. $playbook_path . ' -e "'. input('post.')['variable'] . '"| tee -a '. $logFile;
                ...
                $data = 'ansible -i /tmp/hosts '. $ip['ip'] . ' '. $playbook_path . ' -e "'. input('post.')['variable'] . '"| tee -a '. $logFile;
                $result = array('data'=>$data, 'logfile'=>$logFile);
                Sclient::client($msg=$data);
                return json_encode($result);
                //获取的是主机ip
                ...
                $playbook = input('post.')['playbook'];
                $playbook_path = Playbook::where('name', $playbook)->find()['path'];
                $data = 'ansible -i /tmp/hosts '. input('post.')['ip'] . ' '. $playbook_path . ' -e "'. input('post.')['variable'] . '"| tee -a '. $logFile;
                $result = array('data'=>$data, 'logfile'=>$logFile);
                Sclient::client($msg=$data);
                return json_encode($result);
            }
