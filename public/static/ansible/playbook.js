...
function showplaybook(str){
    $.ajax({
        type: "GET",
        url : "/playbook/playbook_search",
        datatype : 'json',
        data: {'q':str} ,
        success :function (data) {
            console.log(data);
            var datas=eval(data);
            document.getElementById('playbook_search').innerHTML = "";
            for (x in datas)  // x 为属性名
            {
                item = document.createElement('li');
                // item.id = datas[x]['id'];
                item.setAttribute('id',datas[x]['id']);
                item.innerHTML = datas[x]['name'];
                document.getElementById('playbook_search').appendChild(item);
            }
            document.getElementById('playbook_search').appendChild(item);
            document.getElementById('playbook_search').style.border="1px solid #A5ACB2";
            $('ul').on('click','li',function () {
                document.getElementById('playbook').value=this.innerHTML;
                document.getElementById('playbook_search').innerHTML = "";
            });
        }
    })
...
function get_playbook_info() {
    var playbook = $('#playbook').val();
    if(!playbook){
        alert('请先输入剧本名称');
    }else{
          $.get("/playbook/playbook_info?playbook="+playbook,function(data,status){
            document.getElementById('variable_help-block').innerHTML=data;
